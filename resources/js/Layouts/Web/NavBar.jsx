    ...
    return (
        ...
        >
            <Container className="px-10">
                <div className="h-16  flex gap-5 items-center text-nowrap overflow-x-auto hide-scrollbar relative z-40">
                    {categories.map((category) => (
                        <Link
                            key={category.id}
                            href={goTo(category.slug)}
                            onMouseEnter={(e) => handleMouseEnter(e, category)}
                            className="h-full flex items-center text-white whitespace-nowrap text-nowrap hover:text-gray-100"
                        >
                            {category.name}
                ...
                {subCategories?.children?.length > 0 && (
                    <div
                        className="absolute top-full bg-white shadow-lg rounded-b-md overflow-hidden z-40"
                        style={{
